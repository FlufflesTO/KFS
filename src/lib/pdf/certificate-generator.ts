...
function drawHeader(page: PDFPage, width: number, height: number, fonts: CertificateFonts): void {
...
function drawDetails(page: PDFPage, certificateData: CertificateData, startY: number, fonts: CertificateFonts): number {
  let currentY = startY;
...
function drawSignatures(page: PDFPage, width: number, currentY: number, fonts: CertificateFonts): void {
  // Signature area
  page.drawLine({
...
function drawFooter(page: PDFPage, fonts: CertificateFonts): void {
  // Footer
...
export async function generateCertificatePDF(certificateData: CertificateData): Promise<Uint8Array> {
  ...
  drawHeader(page, width, height, fonts);

  const startY = height - 200;
  const currentY = drawDetails(page, certificateData, startY, fonts);

  drawSignatures(page, width, currentY, fonts);
