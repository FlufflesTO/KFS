...
export async function verifyPassword(password: string, storedHash: string): Promise<boolean> {
  ...
  try {
    ...
    const computedHashBase64 = base64UrlEncode(new Uint8Array(derived));
    return timingSafeEqual(textEncoder.encode(computedHashBase64), textEncoder.encode(encodedHash));
  } catch {
    return false;
  }
}
