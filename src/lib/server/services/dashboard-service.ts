...
export class DashboardService {
  ...
  async getAdminStats(): Promise<DashboardStats> {
    const [
      ...
      this.db.prepare(`SELECT SUM(CASE WHEN date(systems.next_due_date) < date('now') THEN 1 ELSE 0 END) AS overdue_systems FROM systems INNER JOIN sites ON sites.id = systems.site_id WHERE systems.deleted_at IS NULL AND sites.deleted_at IS NULL`),
      this.db.prepare(`SELECT SUM(CASE WHEN maintenance_requests.status IN ('New', 'Reviewing') THEN 1 ELSE 0 END) AS open_requests FROM maintenance_requests INNER JOIN sites ON sites.id = maintenance_requests.site_id WHERE sites.deleted_at IS NULL`),
      this.db.prepare(`SELECT
