  ...
  async getAdminStats(): Promise<DashboardStats> {
    const [
      ...
      this.db.prepare(`SELECT
        SUM(CASE WHEN jobs.status IN ('Scheduled', 'In Progress') THEN 1 ELSE 0 END) AS active_jobs,
        SUM(CASE WHEN jobs.status IN ('Scheduled', 'In Progress') AND jobs.assigned_technician_id IS NULL THEN 1 ELSE 0 END) AS unassigned_jobs,
        SUM(CASE WHEN jobs.status IN ('Completed', 'Invoiced') AND jobs.documentation_path IS NULL THEN 1 ELSE 0 END) AS missing_docs
      ...
      this.db.prepare(`SELECT COUNT(*) AS overdue_systems FROM systems INNER JOIN sites ON sites.id = systems.site_id WHERE systems.deleted_at IS NULL AND sites.deleted_at IS NULL AND date(systems.next_due_date) < date('now')`),
      this.db.prepare(`SELECT COUNT(*) AS open_requests FROM maintenance_requests INNER JOIN sites ON sites.id = maintenance_requests.site_id WHERE sites.deleted_at IS NULL AND maintenance_requests.status IN ('New', 'Reviewing')`),
      this.db.prepare(`SELECT
        SUM(CASE WHEN defects.status = 'Open' THEN 1 ELSE 0 END) AS open_defects,
        SUM(CASE WHEN defects.status = 'Open' AND defects.severity = 'Critical' THEN 1 ELSE 0 END) AS critical_defects
      ...
      this.db.prepare(`SELECT
        SUM(CASE WHEN certificates.status = 'Blocked' THEN 1 ELSE 0 END) AS blocked_certs,
        SUM(CASE WHEN certificates.status = 'Valid' THEN 1 ELSE 0 END) AS valid_certs
    ...
    const jobsCounts = getRes<{ active_jobs: number; unassigned_jobs: number; missing_docs: number }>(jobsResult);
    const systemsCounts = getRes<{ overdue_systems: number }>(systemsResult);
    const requestsCounts = getRes<{ open_requests: number }>(requestsResult);
    const defectsCounts = getRes<{ open_defects: number; critical_defects: number }>(defectsResult);
    const certsCounts = getRes<{ blocked_certs: number; valid_certs: number }>(certsResult);

    return {
      activeJobs: Number(jobsCounts.active_jobs ?? 0),
      unassignedJobs: Number(jobsCounts.unassigned_jobs ?? 0),
      overdueSystems: Number(systemsCounts.overdue_systems ?? 0),
      openRequests: Number(requestsCounts.open_requests ?? 0),
      missingDocuments: Number(jobsCounts.missing_docs ?? 0),
      openDefects: Number(defectsCounts.open_defects ?? 0),
      criticalDefects: Number(defectsCounts.critical_defects ?? 0),
      blockedCertificates: Number(certsCounts.blocked_certs ?? 0),
      validCertificates: Number(certsCounts.valid_certs ?? 0)
    };
  }
