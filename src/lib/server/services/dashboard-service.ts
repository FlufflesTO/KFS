...
export class DashboardService {
  ...
  async getAdminStats(): Promise<DashboardStats> {
    const [
      ...
      this.db.prepare(`SELECT SUM(CASE WHEN jobs.status IN ('Scheduled', 'In Progress') THEN 1 ELSE 0 END) AS activeJobs, SUM(CASE WHEN jobs.status IN ('Scheduled', 'In Progress') AND jobs.assigned_technician_id IS NULL THEN 1 ELSE 0 END) AS unassignedJobs, SUM(CASE WHEN jobs.status IN ('Completed', 'Invoiced') AND jobs.documentation_path IS NULL THEN 1 ELSE 0 END) AS missingDocuments FROM jobs INNER JOIN systems ON systems.id = jobs.system_id INNER JOIN sites ON sites.id = systems.site_id WHERE jobs.deleted_at IS NULL AND systems.deleted_at IS NULL AND sites.deleted_at IS NULL`),
      ...
      this.db.prepare(`SELECT SUM(CASE WHEN defects.status = 'Open' THEN 1 ELSE 0 END) AS openDefects, SUM(CASE WHEN defects.status = 'Open' AND defects.severity = 'Critical' THEN 1 ELSE 0 END) AS criticalDefects FROM defects INNER JOIN systems ON systems.id = defects.system_id INNER JOIN sites ON sites.id = systems.site_id WHERE defects.deleted_at IS NULL AND systems.deleted_at IS NULL AND sites.deleted_at IS NULL`),
      this.db.prepare(`SELECT SUM(CASE WHEN certificates.status = 'Blocked' THEN 1 ELSE 0 END) AS blockedCertificates, SUM(CASE WHEN certificates.status = 'Valid' THEN 1 ELSE 0 END) AS validCertificates FROM certificates INNER JOIN systems ON systems.id = certificates.system_id INNER JOIN sites ON sites.id = systems.site_id WHERE certificates.deleted_at IS NULL AND systems.deleted_at IS NULL AND sites.deleted_at IS NULL`)
    ]);
