...
export class DashboardService {
  ...
  async getAdminStats(): Promise<DashboardStats> {
    const [
      ...
      this.db.prepare(`
        SELECT
          COUNT(CASE WHEN jobs.status IN ('Scheduled', 'In Progress') THEN 1 END) AS active_jobs,
          COUNT(CASE WHEN jobs.status IN ('Scheduled', 'In Progress') AND jobs.assigned_technician_id IS NULL THEN 1 END) AS unassigned_jobs,
          COUNT(CASE WHEN jobs.status IN ('Completed', 'Invoiced') AND jobs.documentation_path IS NULL THEN 1 END) AS missing_docs
        FROM jobs
      ...
      this.db.prepare(`
        SELECT
          COUNT(CASE WHEN defects.status = 'Open' THEN 1 END) AS open_defects,
          COUNT(CASE WHEN defects.status = 'Open' AND defects.severity = 'Critical' THEN 1 END) AS critical_defects
        FROM defects
      ...
      this.db.prepare(`
        SELECT
          COUNT(CASE WHEN certificates.status = 'Blocked' THEN 1 END) AS blocked_certs,
          COUNT(CASE WHEN certificates.status = 'Valid' THEN 1 END) AS valid_certs
        FROM certificates
    ...
    const getAgg = (result: unknown, key: string) => {
      const res = result as { results: Record<string, number>[] };
      return res?.results?.[0]?.[key] ?? 0;
    };

    return {
      activeJobs: getAgg(jobsResult, 'active_jobs'),
      unassignedJobs: getAgg(jobsResult, 'unassigned_jobs'),
      overdueSystems: getCount(systemsResult),
      openRequests: getCount(requestsResult),
      missingDocuments: getAgg(jobsResult, 'missing_docs'),
      openDefects: getAgg(defectsResult, 'open_defects'),
      criticalDefects: getAgg(defectsResult, 'critical_defects'),
      blockedCertificates: getAgg(certificatesResult, 'blocked_certs'),
      validCertificates: getAgg(certificatesResult, 'valid_certs')
    };
  }
