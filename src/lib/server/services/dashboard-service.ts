  ...
  async getAdminStats(): Promise<DashboardStats> {
    const [
      ...
      this.db.prepare(`SELECT
        ...
        SUM(CASE WHEN jobs.status IN ('Completed', 'Invoiced') AND jobs.documentation_path IS NULL THEN 1 ELSE 0 END) AS missingDocuments
      ...
      this.db.prepare(`SELECT COUNT(*) AS overdueSystems FROM systems INNER JOIN sites ON sites.id = systems.site_id WHERE systems.deleted_at IS NULL AND sites.deleted_at IS NULL AND date(systems.next_due_date) < date('now')`),
      this.db.prepare(`SELECT COUNT(*) AS openRequests FROM maintenance_requests INNER JOIN sites ON sites.id = maintenance_requests.site_id WHERE sites.deleted_at IS NULL AND maintenance_requests.status IN ('New', 'Reviewing')`),
      this.db.prepare(`SELECT
      ...
      this.db.prepare(`SELECT
        SUM(CASE WHEN certificates.status = 'Blocked' THEN 1 ELSE 0 END) AS blockedCertificates,
        SUM(CASE WHEN certificates.status = 'Valid' THEN 1 ELSE 0 END) AS validCertificates
    ...
    const getStats = (result: unknown) => (result as { results: any[] })?.results?.[0] || {};

    const j = getStats(jobsStats);
    const s = getStats(systemsStats);
    const r = getStats(requestsStats);
    const d = getStats(defectsStats);
    const c = getStats(certsStats);
