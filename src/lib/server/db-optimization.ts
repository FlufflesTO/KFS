...
export async function getDashboardStats(): Promise<DashboardStats> {
  ...
  const [
    ...
    db.prepare(`SELECT COUNT(*) AS overdue_systems FROM systems WHERE deleted_at IS NULL AND date(next_due_date) < date('now')`),
    db.prepare(`SELECT COUNT(*) AS open_requests FROM maintenance_requests WHERE status IN ('New', 'Reviewing')`),
    db.prepare(`SELECT
