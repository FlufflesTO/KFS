...
export async function getClientDashboardData(siteIds: string[]): Promise<ClientDashboardData> {
  ...
  // Execute all queries in a single HTTP request to D1 using db.batch
  // replacing Promise.all with db.batch reduces latency by minimizing database roundtrips
