...
export async function getBatchRecords(batchParams: {
  ...
  if (queryDefs.length > 0) {
    const batchResults = await db.batch(
      queryDefs.map(({ sql, params }) => db.prepare(sql).bind(...params))
    );

    queryDefs.forEach(({ key }, index) => {
      results[key] = batchResults[index]?.results || [];
    });
  }
...
export async function getClientDashboardData(siteIds: string[]): Promise<ClientDashboardData> {
  ...
  // Execute all queries in a single batch for better performance over D1
