...
export async function getClientDashboardData(siteIds: string[]): Promise<ClientDashboardData> {
  ...
  // ⚡ Bolt optimization: Using db.batch instead of Promise.all to consolidate
  // multiple queries into a single HTTP roundtrip to the D1 database, reducing latency.
