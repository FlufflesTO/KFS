...
export async function POST({ request, locals }: APIContext) {
  ...
  try {
    ...
    // Amounts arrive as decimal Rands; convert to integer cents.
    // Reject any non-finite value before storing — NaN/Infinity must never reach D1.
    function parseRandsToCents(value: unknown, field: string): number | null {
    ...
    const sageAmountExVat  = parseRandsToCents(body.sageAmountExVat,  "sageAmountExVat");
    const sageVatAmount    = parseRandsToCents(body.sageVatAmount,    "sageVatAmount");
    const sageAmountIncVat = parseRandsToCents(body.sageAmountIncVat, "sageAmountIncVat");
