...
export async function POST({ params, request, locals }: APIContext): Promise<Response> {
  ...
  try {
    ...
    if (action === "create-member") {
      return await handleCreateMember(db, request, body, user);
    }

    if (action === "update-member") {
      return await handleUpdateMember(db, request, body, user);
    }

    if (action === "delete-member") {
      return await handleDeleteMember(db, request, body, user);
    }
