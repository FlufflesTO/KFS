...
import { cleanBoolean, cleanChoice, cleanDate, cleanId, cleanInt, cleanText, readJson, requireAdmin } from "../../../../lib/server/access";
...
export async function POST({ request, locals }: import('astro').APIContext) {
  ...
  try {
    ...
    if (action === "markInvoiced") {
      return await handleMarkInvoiced(db, body, request, locals);
    }

    return await handleCreateOrUpdate(db, action, body, request, locals);
  } catch (error: unknown) {
...
async function handleMarkInvoiced(db: ReturnType<typeof getDatabase>, body: Record<string, any>, request: Request, locals: import('astro').APIContext['locals']) {
  ...
  const existingFinance: any = await db
...
async function handleCreateOrUpdate(db: ReturnType<typeof getDatabase>, action: string, body: Record<string, any>, request: Request, locals: import('astro').APIContext['locals']) {
