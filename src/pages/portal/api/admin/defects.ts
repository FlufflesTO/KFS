...
async function handleCreate(db: DBType, defectRepository: DefectRepository, request: Request, locals: any, body: any) {
...
async function handleUpdate(db: DBType, defectRepository: DefectRepository, request: Request, locals: any, body: any) {
...
async function handleResolve(db: DBType, defectRepository: DefectRepository, request: Request, locals: any, body: any) {
  const id = cleanId(body.id, "id");
  if (!id) return badRequest("id is required.");
...
async function handleClose(db: DBType, defectRepository: DefectRepository, request: Request, locals: any, body: any) {
  const id = cleanId(body.id, "id");
  if (!id) return badRequest("id is required.");
  const remediationNotes = cleanText(body.remediationNotes, "remediationNotes", { required: false, max: 3000 });
...
async function handleQuoteRequired(db: DBType, request: Request, locals: any, body: any) {
  const id = cleanId(body.id, "id");
  if (!id) return badRequest("id is required.");
...
export async function POST({ request, locals }: import('astro').APIContext) {
  ...
  try {
    ...
    if (action === "create") return await handleCreate(db, defectRepository, request, locals, body);
    if (action === "update") return await handleUpdate(db, defectRepository, request, locals, body);
    if (action === "resolve") return await handleResolve(db, defectRepository, request, locals, body);
    if (action === "close") return await handleClose(db, defectRepository, request, locals, body);
    if (action === "quoteRequired") return await handleQuoteRequired(db, request, locals, body);
