...
async function countExpiredRecords(
  ...
  if (!/^[a-zA-Z0-9_]+$/.test(entityType) || !/^[a-zA-Z0-9_]+$/.test(dateColumn)) {
    console.error(`[DataRetention] Invalid entityType or dateColumn: ${entityType}, ${dateColumn}`);
    return 0;
  }
...
async function deleteExpiredRecords(
  ...
  if (!/^[a-zA-Z0-9_]+$/.test(entityType) || !/^[a-zA-Z0-9_]+$/.test(dateColumn)) {
    return { deleted: 0, batches: 0, error: "Invalid entityType or dateColumn" };
  }
