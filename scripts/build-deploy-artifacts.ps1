...
function Assert-RepoChild {
  ...
  $rootWithSlash = $Root.TrimEnd('\').TrimEnd('/') + [System.IO.Path]::DirectorySeparatorChar
  if (-not ($full -eq $Root -or $full.StartsWith($rootWithSlash, [System.StringComparison]::OrdinalIgnoreCase) -or $full.StartsWith($rootWithSlash.Replace('\', '/'), [System.StringComparison]::OrdinalIgnoreCase) -or $full.StartsWith($rootWithSlash.Replace('/', '\'), [System.StringComparison]::OrdinalIgnoreCase))) {
