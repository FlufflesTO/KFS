...
if (!password || password.length < 12) {
  console.error("Usage: node scripts/hash-password.ts \"minimum-12-character-password\"");
  process.exit(1);
}
...
const derived = await crypto.subtle.deriveBits(
  {
    ...
    iterations
  } as any, // Cast as any because the type definition for salt in Pbkdf2Params expects BufferSource which includes Uint8Array
  keyMaterial,
  256
);
