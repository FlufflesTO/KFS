...
You drive it with **`.cursor/skills/run-kfs/driver.mjs`**, a headless-Chromium
...
> `node .cursor/skills/run-kfs/driver.mjs ...` from PowerShell works.
...
```powershell
node .cursor/skills/run-kfs/driver.mjs shot / home.png
node .cursor/skills/run-kfs/driver.mjs flow
```

`flow` walks `/`, `/services`, `/about`, `/contact`, `/portal/login` and writes a
PNG per page. All screenshots land in `.cursor/skills/run-kfs/shots/`. Each command
...
```powershell
node .cursor/skills/run-kfs/driver.mjs login "client@example.com" "LocalDevPass123!" client
```
...
`.cursor/skills/run-kfs/driver.mjs` — commands: `shot <path> <out.png>`,
